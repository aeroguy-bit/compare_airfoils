...
1. The user enters one or more NACA airfoil numbers.
2. The user specifies:

   * Minimum angle of attack
   * Maximum angle of attack
   * Reynolds number
3. The program creates a range of angles of attack.
4. For each airfoil and every angle, AeroPy calls XFOIL to compute the aerodynamic coefficients.
5. The lift coefficient ((C_L)) is extracted and stored.
6. Finally, all lift polars are plotted on a single graph for comparison.
